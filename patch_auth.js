...
app.post('/api/auth/google', async (req, res) => {
  ...
  if (isSignup) {
    ...
    if (!foundProfile) {
      ...
      try {
        ...
        const { data } = await supabase
          .from('profiles')
          .select('*');
        // Fallback filter in JS if jsonb query is complex, or use correct jsonb arrow
        if (data) {
          const match = data.find(p => p.social_links && p.social_links.email === cleanEmail);
          if (match) {
            foundProfile = match;
    ...
    // Fallback: if not found, we could try finding by username=email_prefix to support old accounts
    if (!foundProfile) {
